...
pub fn get_active_by_project(conn: &Connection, project_id: &str) -> AppResult<Option<WorkingDraft>> {
    let mut stmt = conn.prepare(
        "SELECT working_draft_id, project_id, latest_body_text, updated_at
         FROM working_drafts
...
pub fn save(conn: &Connection, input: SaveDraftInput) -> AppResult<WorkingDraft> {
    ...
    let draft = if let Some(existing) = get_active_by_project(conn, &input.project_id)? {
        ...
        conn.execute(
            "INSERT INTO working_drafts (working_draft_id, project_id, latest_body_text, updated_at)
             VALUES (?1, ?2, ?3, ?4)",
            params![draft_id, input.project_id, input.body_text, now_rfc3339],
        )?;
    ...
    // Update body text
    conn.execute(
        "UPDATE working_drafts SET latest_body_text = ?1, updated_at = ?2 WHERE working_draft_id = ?3",
        params![input.body_text, now_rfc3339, draft.working_draft_id],
    )?;
