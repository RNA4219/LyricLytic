...
import { getProject, getWorkingDraft, getDraftSections, getVersions, createVersion, deleteProject, deleteVersion, restoreVersion as restoreVersionApi, getFragments, createFragment, Project, LyricVersion, VersionSectionInput } from '../../lib/api';
import { useLLMSettings } from '../../lib/llm';
import { useLanguage } from '../../lib/LanguageContext';
import { useProject } from '../../lib/ProjectContext';
import { usePaneResize, useKeyboardShortcuts, createEditorShortcuts, usePhoneticGuide, useUIState, useBpm, useStyleVocal, useClipboard, useDraftPersistence, BPM_PRESETS } from '../../lib/hooks';
...
function EditorPage() {
  ...
  const { setProjectTitle, setBeforeLeave } = useProject();
  ...
  const handleStyleTextChange = useCallback((text: string) => {
    setStyleText(text);
    queueAutoSave(sections, { styleText: text });
  }, [sections, queueAutoSave]);

  const handleVocalTextChange = useCallback((text: string) => {
    setVocalText(text);
    queueAutoSave(sections, { vocalText: text });
  }, [sections, queueAutoSave]);

  const handleBpmPresetChangeWrapper = useCallback((value: string) => {
    handleBpmPresetChange(value);
    const nextBpm = value === 'custom' ? bpmValue : Number(value);
    queueAutoSave(sections, { bpm: nextBpm });
  }, [bpmValue, handleBpmPresetChange, queueAutoSave, sections]);
  ...
  const rhymeGenerationRef = useRef(0);
  const rhymeRunningRef = useRef(false);
  const pendingRhymeRef = useRef<{ text: string; generation: number } | null>(null);

  const drainRhymeQueue = useCallback(async () => {
    if (rhymeRunningRef.current) return;
    rhymeRunningRef.current = true;
    try {
      while (pendingRhymeRef.current) {
        const request = pendingRhymeRef.current;
        pendingRhymeRef.current = null;
        try {
          const rows = await analyzeRhymeGuideRows(request.text);
          if (request.generation === rhymeGenerationRef.current) {
            setPhoneticGuideRows(rows);
          }
        } catch (error) {
          if (request.generation === rhymeGenerationRef.current) {
            console.warn('Failed to load Sudachi rhyme guide. Falling back to local analysis.', error);
            setPhoneticGuideRows(buildFallbackRhymeGuideRows(request.text));
          }
        }
      }
    } finally {
      rhymeRunningRef.current = false;
      if (pendingRhymeRef.current) {
        void drainRhymeQueue();
      }
    }
  }, [setPhoneticGuideRows]);

  useEffect(() => {
    const generation = ++rhymeGenerationRef.current;
    const timer = window.setTimeout(() => {
      pendingRhymeRef.current = { text: editorValue, generation };
      void drainRhymeQueue();
    }, 400);
    return () => window.clearTimeout(timer);
  }, [drainRhymeQueue, editorValue]);
  ...
  return (
    <div className={`editor-workspace ${isLeftPaneVisible ? '' : 'left-pane-hidden'}`}>
          ...
          <div className="editor-pane-header">
            <div className="editor-header-main">
              <div className="editor-header-meta">
                ...
                {(saving || hasPendingSave) && (
                  <span className="autosave-info" role="status">
                    {t('savingDraft')}
                  </span>
                )}
                {!saving && !hasPendingSave && lastSaved && (
            ...
            {(error || autoSaveError) && (
              <div className="editor-error-banner" role="alert">
                <span>{error ?? t('autoSaveFailed')}</span>
                {autoSaveError && (
                  <button type="button" onClick={() => void retryAutoSave()}>
                    {t('retryAutoSave')}
                  </button>
                )}
              </div>
            ...
            <PhoneticGuidePanel
              height={phoneticGuideHeight}
              rows={phoneticGuideRows}
              sourceLabel={rhymeGuideSourceLabel}
              onResizeStart={handlePhoneticGuideResizeStart}
              renderGuideValue={renderGuideValue}
              t={t}
            />
          </div>
        </div>
      ...
      <RightStackPane
        paneRef={rightPaneRef as RefObject<HTMLElement>}
        width={rightPaneWidth}
        sectionPaneHeight={sectionPaneHeight}
        sections={sections}
        ...
        sectionDragPreview={sectionDragPreview}
        onSectionPaneResizeStart={handleSectionPaneResizeStart}
